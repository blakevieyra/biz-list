-- AllConnect: run this entire file in Supabase Dashboard → SQL Editor → Run
...
create policy "Platinum users create campaigns" on public.marketing_campaigns for insert with check (
  auth.uid() = user_id
  and exists (select 1 from public.profiles p where p.id = auth.uid() and p.plan_tier::text = 'platinum')
);
...
create policy "Paid users can insert assessments"
  ...
  with check (
    ...
    and exists (
      ...
      where p.id = auth.uid() and p.plan_tier::text in ('pro', 'platinum')
    )
  );
...
begin
  ...
  set engagement_score = engagement_score + 1, is_trending = (engagement_score + 1) >= 5
...
select enumlabel as plan_tier from pg_enum e
